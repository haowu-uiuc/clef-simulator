...
public class Main_MaxDamageEvaluator_20150930 {
	...
	public static void main(String[] args) throws Exception {
		...
		expName = "20150930_MaxDamageEvaluator";
		...
		UniAttackRateFlowGenerator flowGenerator = new RandomFlowGenerator(linkCapacity,
				timeInterval,
				packetSize,
				numOfSmallFlows,
				numOfLargeFlows,
		...
		int maxAtkRate = (int) (eardet.getGammaH() * 1.5);
		int minAtkRate = (int) (eardet.getGammaL() / 1.5);
		...
		for (String detectorName : resultMap.keySet()) {
			...
			for (int i = 0; i < maxDamageEvaluator.getNumOfRepeatRounds(); i++) {
				System.out.println("Detector: " + detectorName + "\tRound: "
						+ i + "\tRate: " + eardetResult.get(i).atkRate
						+ "\t Counter: " + eardetResult.get(i).numOfCounter
						+ ", Max Damage: " + eardetResult.get(i).maxDamage);
			}
		}
	}
