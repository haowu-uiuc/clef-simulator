...
public class AMFDetector extends MultistageFilter {
	...
	public boolean processPacket(Packet packet) throws Exception {
		...
		// blacklist the flow if it should be.
		if (shouldBlackList) {
	...
	private void initState() {
		...
		for (int i = 0; i < numOfStages; i++) {
			...
			for (int j = 0; j < sizeOfStage; j++) {
				tmpStage.add(new LeakyBucket(threshold, drainRate, linkCapacity));
			}
			stages.add(tmpStage);
		}
	}
	...
	public void logConfig(Logger logger) throws IOException {
		...
		logger.logConfigMsg("Leaky bucket drain rate: " + getDrainRate() + " Byte / sec \n");
